...
for (input in inputs) {
  ...
  for (pol in pollutants) {
    ...
    for (dist in districts)  {
      ...
      data_sub[, pol_lag_highest := shift(get(pol), n = pol_lag_highest_rr[[pol]], type = "lag")]
    ...
    meta_pred_tot <- crosspred(do.call(onebasis, c(list(x=0:max_pol), argvar_pol)), coef=coef(meta_tot), vcov=vcov(meta_tot), cen=0, model.link="log") #
    
    # Store overall meta-analysis results in a list
    results_list[[input]][[pol]]$meta_tot <- meta_tot
    results_list[[input]][[pol]]$meta_pred_tot <- meta_pred_tot
    ...
    results_list[[input]][[pol]]$results_lag <- results_lag
    results_list[[input]][[pol]]$meta_lag <- meta_lag
    results_list[[input]][[pol]]$meta_pred_lag <- meta_pred_lag
    all_results_lag <- rbind(all_results_lag, results_lag)
  }
}

# Save results
saveRDS(results_list, file = "result_list.rds") 
write.csv(all_results_lag, file = "RR_results.csv", row.names = FALSE)
